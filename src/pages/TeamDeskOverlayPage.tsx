...
/**
 * Same desk as shoutcasters — naval frame + phone cam in the hole.
 * `/overlay/team/blue` · `/overlay/team/red`
 * Add `?code=CME24` for domain PeerJS phones.
 */
export default function TeamDeskOverlayPage() {
  const { side = 'blue' } = useParams()
  const [params] = useSearchParams()
  const preview = params.get('preview') === '1'
  const slotId: TeamCamId = side === 'red' ? 'red' : 'blue'
  ...
  const teamName = useCamsStore((s) =>
    slotId === 'blue' ? s.blueName : s.redName,
  )
  ...
  const title = teamName.trim() || (slotId === 'blue' ? 'BLUE TEAM' : 'RED TEAM')
  const subtitle =
    slotId === 'blue' ? 'BLUE SIDE · HOMEBASE' : 'RED SIDE · HOMEBASE'
  ...
  return (
    <div
      className={`sc-shell${preview ? ' is-preview' : ''} td-shell side-${slotId}`}
      ref={shellRef}
    >
      <div
        className={`overlay-root sc-stage${preview ? ' is-preview' : ''} td-stage side-${slotId}`}
        ref={stageRef}
      >
        ...
        {/* Phone cam fills the desk hole */}
        <div className={`td-hole side-${slotId}`} aria-label={`${title} camera`}>
          <video
            ref={videoRef}
            playsInline
            autoPlay
          ...
          />
          {!live && (
            <div className="td-hole-wait">
              <span className="td-hole-kicker">{matchName}</span>
              <strong>{title}</strong>
              <span>
                {status === 'error'
                  ? 'Reconnecting…'
        ...
        <div className="sc-frame" aria-hidden>
          <svg className="sc-frame-svg" viewBox="0 0 1448 576" preserveAspectRatio="none">
            <defs>
              <linearGradient id={`td-frame-edge-${slotId}`} x1="0" y1="0.5" x2="1" y2="0.5">
                {slotId === 'blue' ? (
                  <>
                    <stop offset="0" stopColor="#1e5cff" />
                    <stop offset="0.35" stopColor="#9fd0ff" />
                  ...
                  <>
                    ...
                    <stop offset="0.35" stopColor="#ff9aa6" />
                    <stop offset="0.7" stopColor="#ffffff" />
                    <stop offset="1" stopColor="#e11d2e" />
                  </>
                )}
              </linearGradient>
              <filter id={`td-frame-blur-${slotId}`} x="-4%" y="-8%" width="108%" height="116%">
                <feGaussianBlur stdDeviation="6" />
              </filter>
            </defs>
            <path
              className="sc-frame-glow-path"
              d={FRAME_PATH}
              filter={`url(#td-frame-blur-${slotId})`}
              style={{ stroke: `url(#td-frame-edge-${slotId})` }}
            />
            <path className="sc-frame-body-path" d={FRAME_PATH} />
            <path
              className="sc-frame-edge-path"
              d={FRAME_PATH}
              style={{ stroke: `url(#td-frame-edge-${slotId})` }}
            />
            <path
              className="sc-frame-run-path"
              d={FRAME_PATH}
              pathLength={1000}
              style={{ stroke: `url(#td-frame-edge-${slotId})` }}
            />
        ...
        <div className={`sc-live${live ? ' is-on' : ''}`}>
          <span className="sc-live-dot" />
          <span className="sc-live-word">{live ? 'LIVE' : 'STANDBY'}</span>
        </div>
        ...
        <header className="sc-head">
          ...
          <div className="sc-event">
            ...
            <span className="sc-event-sub">
              YOUNG SAILORS CLUB <i /> {slotId === 'blue' ? 'BLUE DESK' : 'RED DESK'}
            </span>
          </div>
        </header>
        ...
        <footer className="sc-footer">
          ...
          <div className="sc-footer-mid">
            ...
            <span className="sc-footer-onair">TEAM CAM ON AIR</span>
            <span className="sc-chev">
              <b>›</b>
              <b>›</b>
            </span>
